...
RETURN_CODE_ENUM_TYPE example_11(void){
	...
	while (!scanf_s("%f", &item_1_weight)) {
		printf("Bad input, try again");
	}
	printf("Item one quantity?");
	while (!scanf_s("%d", &item_1_quantity)) {
		printf("Bad input, try again");
	}
	printf("Item two weight?");
	while (!scanf_s("%f", &item_2_weight)) {
		printf("Bad input, try again");
	}
	printf("Item two quantity?");
	while (!scanf_s("%d", &item_2_quantity)) {
		printf("Bad input, try again");
	}

	total_items = item_1_quantity + item_2_quantity;
	average_value = ((item_1_weight * item_1_quantity) + (item_2_weight * item_2_quantity)) / total_items;
...
RETURN_CODE_ENUM_TYPE example_12(void){
	...
	while (!scanf_s("%d", &employee_id)) {
		printf("Bad input, try again");
	}
	printf("Total hours employee worked?");
	while (!scanf_s("%d", &total_hours_worked)) {
		printf("Bad input, try again");
	}
	printf("Employee hourly rate?");
	while (!scanf_s("%d", &rate_per_hour)) {
		printf("Bad input, try again");
	}
